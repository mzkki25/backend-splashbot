...
def init_or_update_chat(chat_session, user_id, prompt, file_id_input):
...
async def generate_response(chat_option, prompt, file_id_input, last_response):
    file_url = None
    references = None

    if chat_option == "General Macroeconomics":
        if file_id_input:
            file_url, response = handle_file_prompt(prompt, file_id_input, last_response)
        else:
            response, references = handle_web_prompt(prompt, last_response)
    else:
        response = handle_custom_model(chat_option, prompt)
        file_id_input = None

    return response, file_url, references

def handle_file_prompt(prompt, file_id_input, last_response):
    ...
    if 'application/pdf' in content_type:
        ...
        if len(pdf_text) > 1000:
            relevant_text = find_relevant_chunks(pdf_text, prompt, chunk_size=650, top_k=5)
        else:
...
def handle_web_prompt(prompt, last_response):
...
def handle_custom_model(chat_option, prompt):
    model_map = {
...
def save_chat_messages(chat_ref, chat_session, prompt, response, file_id_input, references):
    chat_ref.update({
